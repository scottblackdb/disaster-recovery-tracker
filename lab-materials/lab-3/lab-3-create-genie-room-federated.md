# Lab 3 (Federated): Build a Genie Agent for the FEMA Claims Data
...
3. When prompted, name the agent `FEMA Claims <Your Name>`.
4. Browse to `fema_lakebase` → `public` and add **all four** tables:
  - `claims`
  - `fema_categories`
  - `documents`
  - `claim_status_history`
5. Make sure the agent's SQL warehouse is set to a **Serverless** warehouse.
...
- **Genie returns a permission error on every question:** The space's SQL warehouse must have **serverless compute enabled** — federated Lakebase catalogs can't be queried by a non-serverless warehouse. Change it in the agent settings.
